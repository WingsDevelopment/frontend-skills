---
name: web3-robust-formatting
description: Robust formatting/normalization wrappers and diagnostics patterns for token amounts, values, and percentages.
---

# web3-robust-formatting

1. Read `references/robust-formatting.md` before editing formatter/mapper logic.
2. Pair this skill with `web3-display-components` when rendering values in UI.
3. Keep robust formatting in mapper/mock layers, never inline in JSX.
4. Call robust formatters directly at mapped fields; do not introduce thin helper wrappers that only proxy formatter calls.
5. Preserve diagnostics (`warnings`, `errors`) and pass them forward to display wrappers.
6. Reuse existing wrappers/helpers from this repo instead of ad-hoc formatting utilities.
...
- `src/app/data/vaults/vaults.mapper.ts`
- `src/app/data/vaults/vaults.fetch.ts`
- `src/app/data/vaults/vaults.types.ts`
...
- Do not pass already-percent-scaled values into `robustFormatPercentToViewPercent` unless you intentionally adjust with `multiplier`/`divider`.
